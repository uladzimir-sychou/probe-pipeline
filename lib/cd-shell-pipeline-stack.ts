...
export class CdPipelineStack extends cdk.Stack {
  ...
  readDeploymentState(): DeploymentState {
    const fileContents = fs.readFileSync('deployment-state/deployment-state.yml', 'utf8');
    return (yaml.load(fileContents) as DeploymentState);
  }
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const deploymentState: DeploymentState = this.readDeploymentState();
    console.log(JSON.stringify(deploymentState));
    ...
    const lambdas = deploymentState.dev.lambdas ? deploymentState.dev.lambdas : [];
        ...
        phases: {
          build: {
            commands: [
              ...
              'printenv',
              'yq --version',
              `artifact=$(yq e '.dev.lambdas[] | select(.name == "GreetingLambda") | .artifact' deployment-state.yml)`,
              'echo $artifact',
